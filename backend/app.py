import os, json, uuid, threading, logging, time, sqlite3, subprocess, functools
...
API_KEY = os.environ.get('UNTube_API_KEY', '').strip()
MAX_UPLOAD_MB = int(os.environ.get('UNTube_MAX_UPLOAD_MB', '50'))
...
CREATE TABLE IF NOT EXISTS channels (
    ...
    download_mode TEXT DEFAULT 'latest',
...
CREATE TABLE IF NOT EXISTS rules (
    ...
    action TEXT NOT NULL DEFAULT 'tag' CHECK (action IN ('tag', 'reject', 'keep')),
...
CREATE TABLE IF NOT EXISTS watch_later (
    video_id TEXT PRIMARY KEY,
    bucket TEXT NOT NULL DEFAULT 'today' CHECK (bucket IN ('today', 'tonight', 'tomorrow', 'weekend')),
...
_DEFAULT_SETTINGS = {
    ...
    'sidebar_order': 'feed,playlists,channels,rejected,watch_later,settings',
    'dearrow_enabled': False,
    'dearrow_titles': True,
    'dearrow_thumbnails': False,
    'comments_enabled': False,
    'smart_retention_days': 0,
    'smart_retention_protect_liked': True,
    'playlist_countdown': 5,
}
...
def init_db():
    ...
    for col, defn in [
        ...
        try:
            ...
            logger.warning(f"Migration error (videos.{col}): {e}")
    for col, defn in [
        ('quality', 'TEXT DEFAULT NULL'),
        ('notify', 'INTEGER DEFAULT 0'),
        ('auto_delete_days', 'INTEGER DEFAULT NULL'),
        ...
        try:
            ...
            logger.warning(f"Migration error (channels.{col}): {e}")
    try:
        conn.execute("ALTER TABLE watch_progress ADD COLUMN updated_at TEXT DEFAULT ''")
        conn.commit()
    except sqlite3.OperationalError:
        pass
    except Exception as e:
        logger.warning(f"Migration error (watch_progress.updated_at): {e}")
    try:
        conn.execute('CREATE INDEX IF NOT EXISTS idx_videos_sort_date ON videos(sort_date DESC)')
        conn.commit()
    except Exception as e:
        logger.warning(f"Migration error (idx_videos_sort_date): {e}")
    try:
        conn.execute("ALTER TABLE videos ADD COLUMN status TEXT DEFAULT 'active'")
        conn.commit()
    except sqlite3.OperationalError:
        pass
    except Exception as e:
        logger.warning(f"Migration error (videos.status): {e}")
    try:
        conn.execute("ALTER TABLE channels ADD COLUMN playback_speed REAL DEFAULT 1.0")
        conn.commit()
    except sqlite3.OperationalError:
        pass
    except Exception as e:
        logger.warning(f"Migration error (channels.playback_speed): {e}")
    import shutil
    legacy_data = Path('/app/internal-data')
    if legacy_data.exists() and legacy_data.is_dir():
        legacy_db = legacy_data / 'db.sqlite'
        if legacy_db.exists() and not DB_PATH.exists():
            try:
                shutil.copy2(legacy_db, DB_PATH)
                for suffix in ('-shm', '-wal'):
                    src = legacy_data / f'db.sqlite{suffix}'
                    if src.exists():
                        shutil.copy2(src, DATA_DIR / f'db.sqlite{suffix}')
                for subdir in ('thumbcache', 'storyboards'):
                    src = legacy_data / subdir
                    if src.exists() and src.is_dir():
                        dest = DATA_DIR / subdir
                        if not dest.exists():
                            shutil.copytree(src, dest)
                shutil.rmtree(legacy_data, ignore_errors=True)
                logger.info("Migrated database from legacy internal path to data volume")
            except Exception as e:
...
def _row_to_video(row):
    ...
    if 'scheduled_at' in keys:
        d['scheduled_at'] = row['scheduled_at']
    return d
    ...
    with conn:
        conn.execute(
            ...
            (ch['id'], ch.get('name', ''), ch.get('url', ''), ch.get('thumbnail', ''),
             ch.get('subscriber_count'), ch.get('download_mode', 'latest'),
...
def update_channel(cid, updates):
    ...
    if 'playback_speed' in updates:
        sets.append('playback_speed = ?')
        vals.append(max(0.25, min(4.0, float(updates['playback_speed']))))
    if not sets:
        return
    vals.append(cid)
    conn = _get_conn()
...
def get_videos(channel_id=None, include_ghosts=True, page=1, per_page=100, status=None):
    conds, vals = [], []
    if channel_id:
    ...
    sort = 'ORDER BY downloaded_at DESC'
...
def set_watch_progress(vid, t):
    ...
    with conn:
        if t <= 0:
            ...
            conn.execute(
                'INSERT OR REPLACE INTO watch_progress (video_id, time, updated_at) VALUES (?,?,?)',
                (vid, t, datetime.now(timezone.utc).isoformat()))


def get_continue_watching(limit=12):
    """Videos that are partially watched (>= 30s in, < 95% done), most recent first."""
    rows = _get_conn().execute(
        'SELECT v.*, wp.time AS wp_time FROM watch_progress wp'
        ' JOIN videos v ON v.id = wp.video_id'
        ' WHERE v.ghost = 0 AND v.file_path != \'\''
        ' AND wp.time >= 30 AND (v.duration <= 0 OR wp.time / v.duration < 0.95)'
        ' ORDER BY wp.updated_at DESC LIMIT ?', (limit,)).fetchall()
    return [_row_to_video(r) for r in rows]
...
_rate_last_times: dict[int, float] = {}

_hls_semaphore = threading.Semaphore(8)


def _rate_limit(min_delay=2.0):
    tid = threading.get_ident()
    with _rate_lock:
        last = _rate_last_times.get(tid, 0)
    ...
    if elapsed < min_delay:
        ...
        time.sleep(wait)
    with _rate_lock:
        _rate_last_times[tid] = time.time()


def _retry(fn, retries=3, delay=5, backoff=2, label=''):
    last_err = None
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                ...
                time.sleep(wait)
    logger.error(f"  {label} failed after {retries} attempts: {last_err}")
    raise last_err
...
def get_channel_info(url):
    try:
        ...
        info = _retry(_fetch, label=f'get_channel_info({url})')
        return {
...
def fetch_channel_videos(channel_url, max_items=30):
    ...
    try:
        ...
        info = _retry(_fetch, label=f'fetch_channel_videos({channel_url})')
    ...
    try:
        def _fetch():
            with yt_dlp.YoutubeDL({**base_opts(), 'skip_download': True}) as ydl:
                return ydl.extract_info(url, download=False)
        info = _retry(_fetch, retries=2, delay=3, label=f'age_check({vid_id})')
...
def _worker(stub, channel, quality, skip_shorts, max_days_old):
    ...
    add_video(entry)
    _apply_rules_to_video(entry['id'])
    ...
    try:
        ...
        for ch in get_channels():
            ...
            if mode == 'latest' or (mode == 'all' and has_videos and max_days > 0):
                ...
                fetch_count = s.get('check_fetch_count', 100)

            if not ch.get('url'):
                logger.info(f"  {ch['name']}: no URL — skipping fetch")
                continue
...
def fetch_video_metadata(vid):
    ...
    try:
        def _fetch():
            with yt_dlp.YoutubeDL({**base_opts(), 'skip_download': True}) as ydl:
                return ydl.extract_info(url, download=False)
        info = _retry(_fetch, retries=2, delay=3, label=f'metadata({vid})')
        if info:
                return {
...
def r_vids():
    ...
    vids, total = get_videos(channel_id=cid, include_ghosts=include_ghosts, page=page, per_page=per_page, status=status)
...
def r_import_opml():
    ...
    try:
        ...
        return jsonify({'error': 'Invalid OPML/XML'}), 400
    channel_ids = set()
    for outline in root.iter('outline'):
        xml_url = outline.get('xmlUrl', '')
        m = re.search(r'channel_id=([A-Za-z0-9_-]+)', xml_url)
        if m:
            channel_ids.add(m.group(1))
            continue
        html_url = outline.get('htmlUrl', '')
        m2 = re.search(r'channel/([A-Za-z0-9_-]+)', html_url)
        if m2:
            channel_ids.add(m2.group(1))
    imported = 0
    skipped = 0
    for cid in channel_ids:
        if channel_exists(cid):
            skipped += 1
            continue
        name = cid
        for outline in root.iter('outline'):
            ...
            if m and m.group(1) == cid:
                name = outline.get('text', '') or outline.get('title', '') or cid
                break
        add_channel({'id': cid, 'name': name, 'url': f'https://www.youtube.com/channel/{cid}'})
        imported += 1
    return jsonify({'ok': True, 'imported': imported, 'skipped': skipped, 'total_found': len(channel_ids)})
...
def r_continue(): return jsonify(get_continue_watching())
...
def r_upd_settings():
    ...
    if 'card_density' in d and d['card_density'] in ('compact', 'comfortable', 'spacious'):
        s['card_density'] = d['card_density']
    if 'watched_style' in d and d['watched_style'] in ('dim', 'strikethrough', 'hidden'):
        s['watched_style'] = d['watched_style']
    if 'incognito' in d:
        s['incognito'] = bool(d['incognito'])
    if 'sponsorblock_enabled' in d:
        s['sponsorblock_enabled'] = bool(d['sponsorblock_enabled'])
    if 'sponsorblock_categories' in d:
        s['sponsorblock_categories'] = str(d['sponsorblock_categories'])
    if 'check_fetch_count' in d:
        s['check_fetch_count'] = max(1, min(200, int(d['check_fetch_count'])))
    if 'sidebar_order' in d:
        s['sidebar_order'] = str(d['sidebar_order'])
    if 'dearrow_enabled' in d:
        s['dearrow_enabled'] = bool(d['dearrow_enabled'])
    if 'dearrow_titles' in d:
        s['dearrow_titles'] = bool(d['dearrow_titles'])
    if 'dearrow_thumbnails' in d:
        s['dearrow_thumbnails'] = bool(d['dearrow_thumbnails'])
    if 'comments_enabled' in d:
        s['comments_enabled'] = bool(d['comments_enabled'])
    if 'smart_retention_days' in d:
        s['smart_retention_days'] = max(0, int(d['smart_retention_days']))
    if 'smart_retention_protect_liked' in d:
        s['smart_retention_protect_liked'] = bool(d['smart_retention_protect_liked'])
    if 'playlist_countdown' in d:
        s['playlist_countdown'] = max(0, min(30, int(d['playlist_countdown'])))
    save_settings(s)
    return jsonify(s)
...
def r_playlists():
    conn = _get_conn()
    rows = conn.execute('SELECT * FROM playlists ORDER BY updated_at DESC, name').fetchall()
    result = []
    for row in rows:
        pl = dict(row)
        count = conn.execute('SELECT COUNT(*) FROM playlist_videos WHERE playlist_id = ?', (pl['id'],)).fetchone()[0]
...
def r_create_playlist():
    ...
    conn = _get_conn()
    conn.execute('INSERT INTO playlists (id, name, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?)',
                 (pl_id, name, d.get('description', ''), now, now))
    conn.commit()
...
@require_api_key
def r_update_playlist(pl_id):
    d = request.json or {}
    conn = _get_conn()
    pl = conn.execute('SELECT * FROM playlists WHERE id = ?', (pl_id,)).fetchone()
    if not pl:
        return jsonify({'error': 'Playlist not found'}), 404
    name = (d.get('name') or pl['name']).strip()
    desc = d.get('description', pl['description'])
    now = datetime.now().isoformat()
    conn.execute('UPDATE playlists SET name = ?, description = ?, updated_at = ? WHERE id = ?',
                 (name, desc, now, pl_id))
    conn.commit()
    pl = conn.execute('SELECT * FROM playlists WHERE id = ?', (pl_id,)).fetchone()
...
@require_api_key
def r_delete_playlist(pl_id):
    conn = _get_conn()
    conn.execute('DELETE FROM playlists WHERE id = ?', (pl_id,))
    conn.execute('DELETE FROM playlist_videos WHERE playlist_id = ?', (pl_id,))
    conn.commit()
    return jsonify({'ok': True})


@app.route('/api/playlists/<pl_id>/videos', methods=['GET'])
def r_playlist_videos(pl_id):
    conn = _get_conn()
    pl = conn.execute('SELECT * FROM playlists WHERE id = ?', (pl_id,)).fetchone()
    if not pl:
        return jsonify({'error': 'Playlist not found'}), 404
    ...
    return jsonify({'playlist': dict(pl), 'videos': videos})


@app.route('/api/playlists/<pl_id>/videos', methods=['POST'])
@require_api_key
def r_add_to_playlist(pl_id):
    d = request.json or {}
    ...
    if not pl:
        return jsonify({'error': 'Playlist not found'}), 404
    now = datetime.now().isoformat()
    max_pos = conn.execute('SELECT COALESCE(MAX(position), -1) FROM playlist_videos WHERE playlist_id = ?', (pl_id,)).fetchone()[0]
    added = 0
    for i, vid in enumerate(video_ids):
        try:
            cur = conn.execute('INSERT OR IGNORE INTO playlist_videos (playlist_id, video_id, position, added_at) VALUES (?, ?, ?, ?)',
                         (pl_id, vid, max_pos + 1 + i, now))
            if cur.rowcount > 0:
                added += 1
        except Exception:
            pass
...
@app.route('/api/playlists/<pl_id>/videos/<video_id>', methods=['DELETE'])
@require_api_key
def r_remove_from_playlist(pl_id, video_id):
    conn = _get_conn()
    conn.execute('DELETE FROM playlist_videos WHERE playlist_id = ? AND video_id = ?', (pl_id, video_id))
    conn.execute('UPDATE playlists SET updated_at = ? WHERE id = ?', (datetime.now().isoformat(), pl_id))
    conn.commit()
    return jsonify({'ok': True})


@app.route('/api/playlists/<pl_id>/reorder', methods=['PUT'])
@require_api_key
def r_reorder_playlist(pl_id):
    d = request.json or {}
    order = d.get('order', [])
    if not order:
    ...
    for i, vid in enumerate(order):
        conn.execute('UPDATE playlist_videos SET position = ? WHERE playlist_id = ? AND video_id = ?', (i, pl_id, vid))
    conn.execute('UPDATE playlists SET updated_at = ? WHERE id = ?', (now, pl_id))
    conn.commit()
    return jsonify({'ok': True})


@app.route('/api/playlists/for-video/<video_id>', methods=['GET'])
def r_playlists_for_video(video_id):
    conn = _get_conn()
    rows = conn.execute('''
...
def r_tags():
    ...
    for row in rows:
        t = dict(row)
        t['video_count'] = conn.execute('SELECT COUNT(*) FROM video_tags WHERE tag_id = ?', (t['id'],)).fetchone()[0]
        t['channel_count'] = conn.execute('SELECT COUNT(*) FROM channel_tags WHERE tag_id = ?', (t['id'],)).fetchone()[0]
        result.append(t)
    return jsonify(result)
...
@require_api_key
def r_update_tag(tag_id):
    d = request.json or {}
    conn = _get_conn()
    if 'name' in d:
        conn.execute('UPDATE tags SET name = ? WHERE id = ?', ((d['name'] or '').strip(), tag_id))
    if 'color' in d:
        conn.execute('UPDATE tags SET color = ? WHERE id = ?', ((d['color'] or '').strip(), tag_id))
    conn.commit()
    return jsonify({'ok': True})
...
@app.route('/api/videos/<vid>/tags', methods=['GET'])
def r_video_tags(vid):
    conn = _get_conn()
    rows = conn.execute('''
...
@app.route('/api/videos/<vid>/tags', methods=['POST'])
@require_api_key
def r_add_video_tag(vid):
    tag_id = (request.json or {}).get('tag_id')
    if not tag_id:
        return jsonify({'error': 'tag_id required'}), 400
    conn = _get_conn()
    conn.execute('INSERT OR IGNORE INTO video_tags (video_id, tag_id) VALUES (?, ?)', (vid, tag_id))
    conn.commit()
    return jsonify({'ok': True})


@app.route('/api/videos/<vid>/tags/<int:tag_id>', methods=['DELETE'])
@require_api_key
def r_remove_video_tag(vid, tag_id):
    conn = _get_conn()
    conn.execute('DELETE FROM video_tags WHERE video_id = ? AND tag_id = ?', (vid, tag_id))
    conn.commit()
    return jsonify({'ok': True})


@app.route('/api/channels/<cid>/tags', methods=['GET'])
def r_channel_tags(cid):
    conn = _get_conn()
    rows = conn.execute('''
        SELECT t.* FROM tags t
...
@app.route('/api/channels/<cid>/tags', methods=['POST'])
@require_api_key
def r_add_channel_tag(cid):
    tag_id = (request.json or {}).get('tag_id')
    if not tag_id:
        return jsonify({'error': 'tag_id required'}), 400
    conn = _get_conn()
    conn.execute('INSERT OR IGNORE INTO channel_tags (channel_id, tag_id) VALUES (?, ?)', (cid, tag_id))
    conn.commit()
    return jsonify({'ok': True})


@app.route('/api/channels/<cid>/tags/<int:tag_id>', methods=['DELETE'])
@require_api_key
def r_remove_channel_tag(cid, tag_id):
    conn = _get_conn()
    conn.execute('DELETE FROM channel_tags WHERE channel_id = ? AND tag_id = ?', (cid, tag_id))
    conn.commit()
    return jsonify({'ok': True})


@app.route('/api/tags/<int:tag_id>/videos', methods=['GET'])
def r_videos_by_tag(tag_id):
    page = max(1, int(request.args.get('page', 1)))
    per_page = max(1, min(200, int(request.args.get('per_page', 100))))
    conn = _get_conn()
    total = conn.execute('SELECT COUNT(*) FROM video_tags WHERE tag_id = ?', (tag_id,)).fetchone()[0]
    rows = conn.execute('''
        ...
        LIMIT ? OFFSET ?
    ''', (tag_id, per_page, (page - 1) * per_page)).fetchall()
    return jsonify({'videos': [_row_to_video(r) for r in rows], 'total': total})
...
def r_create_rule():
    ...
    action = d.get('action', 'tag')
    tag_id = d.get('tag_id')
    conn = _get_conn()
    conn.execute(
        'INSERT INTO rules (name, pattern, match_type, field, action, tag_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
        (name, pattern, match_type, field, action, tag_id, datetime.now().isoformat()))
    conn.commit()
...
@require_api_key
def r_update_rule(rule_id):
    d = request.json or {}
    conn = _get_conn()
    if 'name' in d:
        conn.execute('UPDATE rules SET name = ? WHERE id = ?', ((d['name'] or '').strip(), rule_id))
    if 'pattern' in d:
        conn.execute('UPDATE rules SET pattern = ? WHERE id = ?', ((d['pattern'] or '').strip(), rule_id))
    if 'match_type' in d:
        conn.execute('UPDATE rules SET match_type = ? WHERE id = ?', (d['match_type'], rule_id))
    if 'field' in d:
        conn.execute('UPDATE rules SET field = ? WHERE id = ?', (d['field'], rule_id))
    if 'action' in d:
        conn.execute('UPDATE rules SET action = ? WHERE id = ?', (d['action'], rule_id))
    if 'tag_id' in d:
        conn.execute('UPDATE rules SET tag_id = ? WHERE id = ?', (d['tag_id'], rule_id))
    if 'enabled' in d:
        conn.execute('UPDATE rules SET enabled = ? WHERE id = ?', (1 if d['enabled'] else 0, rule_id))
    conn.commit()
    return jsonify({'ok': True})
...
def _apply_rules_to_video(vid):
    ...
    for rule in rules:
        ...
        if rule['match_type'] == 'regex':
            import re
            try:
                match = bool(re.search(rule['pattern'], title if field == 'title' else desc, re.IGNORECASE))
            except re.error:
                pass
        else:
            text = title if field == 'title' else (desc if field == 'description' else title + ' ' + desc)
            match = pattern in text
        if match:
            if rule['action'] == 'tag' and rule['tag_id']:
                conn.execute('INSERT OR IGNORE INTO video_tags (video_id, tag_id) VALUES (?, ?)', (vid, rule['tag_id']))
            elif rule['action'] == 'reject':
                conn.execute("UPDATE videos SET status = 'rejected' WHERE id = ? AND status = 'active'", (vid,))
            elif rule['action'] == 'keep':
                conn.execute("UPDATE videos SET status = 'active' WHERE id = ? AND status = 'rejected'", (vid,))
    conn.commit()
...
def r_apply_rules():
    conn = _get_conn()
    vids = conn.execute('SELECT id FROM videos WHERE status = ? OR status IS NULL', ('active',)).fetchall()
    count = 0
    for row in vids:
        old_status = conn.execute('SELECT status FROM videos WHERE id = ?', (row['id'],)).fetchone()
        _apply_rules_to_video(row['id'])
        new_status = conn.execute('SELECT status FROM videos WHERE id = ?', (row['id'],)).fetchone()
        if old_status and new_status and old_status['status'] != new_status['status']:
            count += 1
...
@app.route('/api/videos/<vid>/archive', methods=['POST'])
@require_api_key
def r_archive_video(vid):
    conn = _get_conn()
    conn.execute("UPDATE videos SET status = 'rejected' WHERE id = ?", (vid,))
    conn.commit()
    return jsonify({'ok': True})


@app.route('/api/videos/rejected', methods=['GET'])
def r_rejected_videos():
    page = max(1, int(request.args.get('page', 1)))
    per_page = max(1, min(200, int(request.args.get('per_page', 100))))
    conn = _get_conn()
    total = conn.execute("SELECT COUNT(*) FROM videos WHERE status = 'rejected'").fetchone()[0]
    rows = conn.execute(
        "SELECT * FROM videos WHERE status = 'rejected' ORDER BY downloaded_at DESC LIMIT ? OFFSET ?",
...
def r_watch_later():
    ...
    if bucket:
        rows = conn.execute('''
            SELECT v.*, wl.bucket, wl.scheduled_at FROM videos v
            JOIN watch_later wl ON wl.video_id = v.id
            WHERE wl.bucket = ?
            ORDER BY wl.scheduled_at
        ''', (bucket,)).fetchall()
    else:
        rows = conn.execute('''
            SELECT v.*, wl.bucket, wl.scheduled_at FROM videos v
            JOIN watch_later wl ON wl.video_id = v.id
            ORDER BY wl.bucket, wl.scheduled_at
        ''').fetchall()
    return jsonify({'videos': [_row_to_video(r) for r in rows]})


@app.route('/api/watch-later', methods=['POST'])
@require_api_key
def r_add_watch_later():
    d = request.json or {}
    video_id = d.get('video_id', '').strip()
    bucket = d.get('bucket', 'today')
    if not video_id:
        return jsonify({'error': 'video_id required'}), 400
    if bucket not in ('today', 'tonight', 'tomorrow', 'weekend'):
        return jsonify({'error': 'Invalid bucket'}), 400
    conn = _get_conn()
    conn.execute('INSERT OR REPLACE INTO watch_later (video_id, bucket, scheduled_at) VALUES (?, ?, ?)',
                 (video_id, bucket, datetime.now().isoformat()))
    conn.commit()
    return jsonify({'ok': True})


@app.route('/api/watch-later/<vid>', methods=['DELETE'])
@require_api_key
def r_remove_watch_later(vid):
    conn = _get_conn()
    conn.execute('DELETE FROM watch_later WHERE video_id = ?', (vid,))
    conn.commit()
    return jsonify({'ok': True})
...
@require_api_key
def r_clear_watch_later():
    bucket = (request.json or {}).get('bucket')
    conn = _get_conn()
    if bucket:
        conn.execute('DELETE FROM watch_later WHERE bucket = ?', (bucket,))
    else:
        conn.execute('DELETE FROM watch_later')
    conn.commit()
    return jsonify({'ok': True})
...
def r_sponsorblock(vid):
    ...
    import hashlib
    s = get_settings()
    if not s.get('sponsorblock_enabled'):
        return jsonify([])
    categories = (s.get('sponsorblock_categories') or 'sponsor,selfpromo,interaction').split(',')
    try:
        sha = hashlib.sha256(vid.encode()).hexdigest()[:4]
        cats_param = json.dumps(categories)
        url = f'https://sponsor.ajay.app/api/skipSegments/{sha}?categories={urllib.parse.quote(cats_param)}'
        req = _ur.Request(url, headers={'User-Agent': 'UnTube/1.0'})
            ...
            if item.get('videoID') == vid:
                for seg in item.get('segments', []):
                    segments.append({
                        ...
                        'segment': seg.get('segment', [0, 0]),
                        'actionType': seg.get('actionType', 'skip'),
                    })
                break
...
@require_api_key
def r_check():
    if check_running.locked():
        return jsonify({'ok': True, 'note': 'check already in progress'})
    threading.Thread(target=check_for_new_videos, daemon=True).start()
    return jsonify({'ok': True})
...
def r_browse(cid):
    ...
    if cid == 'uncategorized':
        conn = _get_conn()
        cur = conn.execute('SELECT * FROM videos WHERE channel_id = "uncategorized" AND status != "rejected" ORDER BY upload_date DESC, downloaded_at DESC LIMIT ?', (max_items,))
        videos = [_row_to_video(r) for r in cur.fetchall()]
        for v in videos:
            v['status'] = v.get('status', 'downloaded')
...
def r_restore_video(vid):
    video = get_video(vid)
    if video and video.get('status') == 'rejected':
        conn = _get_conn()
        conn.execute("UPDATE videos SET status = 'active' WHERE id = ?", (vid,))
        conn.commit()
        return jsonify({'ok': True})
    if not video:
        return jsonify({'error': 'Not found'}), 404
...
@app.route('/api/dearrow/<vid>', methods=['GET'])
def r_dearrow(vid):
    import urllib.request, urllib.error
    import hashlib
    sha = hashlib.sha256(vid.encode()).hexdigest()[:4]
    result = {'title': None, 'thumbnail': None}
    try:
        req = urllib.request.Request(
            f'https://dearrow.ajay.app/api/getVideo?videoID={vid}',
            headers={'User-Agent': 'UnTube/1.0'}
        )
        with urllib.request.urlopen(req, timeout=5) as resp:
            ...
            if data.get('type') == 'registered':
                ...
                if thumbs:
                    result['thumbnail'] = thumbs[0].get('url', '')
    except Exception:
        pass
    return jsonify(result)
...
@app.route('/api/comments/<vid>', methods=['GET'])
def r_comments(vid):
    import urllib.request, urllib.error
    comments = []
    try:
        url = f'https://returnyoutubedislikeapi.com/votes?videoId={vid}'
        req = urllib.request.Request(url, headers={'User-Agent': 'UnTube/1.0'})
        with urllib.request.urlopen(req, timeout=5) as resp:
            data = json.loads(resp.read().decode())
            comments.append({
                'text': f'👍 {data.get("likes", 0)} likes · 👎 {data.get("dislikes", 0)} dislikes · {data.get("viewCount", 0)} views',
                'author': 'Stats',
                'likes': 0,
            })
    except Exception:
...
@app.route('/api/import/takeout', methods=['POST'])
@require_api_key
def r_import_takeout():
    import csv
    import io
    import re
    if 'file' not in request.files:
    ...
    for line in raw.splitlines():
        m = re.search(r'channel/([A-Za-z0-9_-]+)', line)
        if m:
            channel_ids.add(m.group(1))
            continue
        m2 = re.search(r'Channel ID[:\s]+([A-Za-z0-9_-]+)', line)
        if m2:
            channel_ids.add(m2.group(1))
    imported = 0
    skipped = 0
    for cid in channel_ids:
        if channel_exists(cid):
            skipped += 1
            continue
        add_channel({'id': cid, 'name': cid, 'url': f'https://www.youtube.com/channel/{cid}'})
        imported += 1
    return jsonify({'ok': True, 'imported': imported, 'skipped': skipped, 'total_found': len(channel_ids)})
...
def _smart_retention():
    s = get_settings()
    days = s.get('smart_retention_days', 0)
    if not days or days <= 0:
        return
    conn = _get_conn()
    cutoff = (datetime.now(timezone.utc) - timedelta(days=days)).strftime('%Y%m%d')
    protect_liked = s.get('smart_retention_protect_liked', True)
    query = "SELECT id FROM videos WHERE status = 'active' AND upload_date != '' AND upload_date < ? AND ghost = 0"
    params = [cutoff]
    if protect_liked:
        query += " AND like_count <= 0"
    rows = conn.execute(query, params).fetchall()
    if rows:
        ids = [r['id'] for r in rows]
        placeholders = ','.join(['?' for _ in ids])
        conn.execute(f"UPDATE videos SET status = 'rejected' WHERE id IN ({placeholders})", ids)
        conn.commit()
        logger.info(f"Smart retention: archived {len(ids)} videos older than {days} days")


# ─── Channel Playback Speed ────────────────────────────────────

@app.route('/api/channels/<cid>/speed', methods=['PUT'])
@require_api_key
def r_set_channel_speed(cid):
    d = request.json or {}
    speed = float(d.get('speed', 1.0))
    speed = max(0.25, min(4.0, speed))
    conn = _get_conn()
    conn.execute('UPDATE channels SET playback_speed = ? WHERE id = ?', (speed, cid))
    conn.commit()
    return jsonify({'ok': True, 'speed': speed})


@app.route('/api/channels/<cid>/speed', methods=['GET'])
def r_get_channel_speed(cid):
    ch = get_channel(cid)
    if not ch:
        return jsonify({'speed': 1.0})
    return jsonify({'speed': ch.get('playback_speed', 1.0)})
...
HLS_SEG_DURATION = 4
# How far past the encode frontier a request may be before we restart the
# session at the requested segment instead of waiting for ffmpeg to reach it.
HLS_WAIT_AHEAD = 12
# Cached renditions are garbage-collected after this many seconds idle.
HLS_CACHE_TTL = 3 * 3600
...
def _cleanup_hls():
    ...
    now = time.time()
    with _hls_lock:
        for k in list(_hls_procs):
            info = _hls_procs[k]
            if info['proc'].poll() is not None:
                del _hls_procs[k]          # finished — keep segments as cache
                _hls_semaphore.release()
            elif now - info['started'] > 4 * 3600:
                _kill_hls_proc(info['proc'])   # runaway safety net
                del _hls_procs[k]
                _hls_semaphore.release()
        active_dirs = {v['dir'] for v in _hls_procs.values()}
    hls_root = DATA_DIR / 'hls'
    if hls_root.exists():
        for d in hls_root.iterdir():
            if str(d) in active_dirs or not d.is_dir():
                continue
            try:
                if now - d.stat().st_mtime > HLS_CACHE_TTL:
                    shutil.rmtree(d, ignore_errors=True)
            except OSError:
                pass


def _kill_hls_proc(proc):
    try:
        proc.kill()
        try:
            proc.wait(timeout=5)
...
def _hls_seg_ready(hls_dir, seg_num):
    p = Path(hls_dir) / f'seg_{seg_num:05d}.ts'
    try:
        return p.stat().st_size > 0 and not p.with_suffix('.ts.tmp').exists()
    except OSError:
        return False


def _hls_frontier(hls_dir, start_seg):
    """Highest contiguous segment number produced from start_seg onward."""
    n = start_seg
    while _hls_seg_ready(hls_dir, n):
        n += 1
    return n - 1


def _start_hls_session(key, src, q, hls_dir, start_seg):
    """(Re)start the transcode session at start_seg. Caller holds _hls_lock."""
    import subprocess
    old = _hls_procs.pop(key, None)
    if old:
        _kill_hls_proc(old['proc'])
    hls_dir.mkdir(parents=True, exist_ok=True)
    start_time = start_seg * HLS_SEG_DURATION
    ...
    cmd += [
        '-i', str(src),
        # Cap at source height so a stale higher-than-source quality
        # setting degrades to a same-size transcode instead of upscaling.
        '-vf', f'scale=-2:min({q}\\,ih)',
        '-c:v', 'libx264', '-preset', 'ultrafast',
        '-tune', 'zerolatency',
        '-force_key_frames', f'expr:gte(t,n_forced*{HLS_SEG_DURATION})',
        '-crf', crf, '-threads', '0',
        '-c:a', 'aac', '-b:a', '96k' if q <= 480 else '128k', '-ac', '2',
        '-f', 'hls',
        '-hls_time', str(HLS_SEG_DURATION),
        '-hls_list_size', '0',
    ...
    logger.info(f"HLS session [{key}] start at seg {start_seg} (t={start_time}s)")
    _hls_semaphore.acquire()
    try:
    ...
    def _drain(p, k, sem):
        ...
        sem.release()
    threading.Thread(target=_drain, args=(proc, key, _hls_semaphore), daemon=True).start()
    return proc
...
def r_hls_playlist(vid, q):
    ...
    num_segments = math.ceil(duration / HLS_SEG_DURATION)
    m3u8 = '#EXTM3U\n#EXT-X-VERSION:3\n#EXT-X-TARGETDURATION:{}\n'.format(HLS_SEG_DURATION + 1)
    m3u8 += '#EXT-X-MEDIA-SEQUENCE:0\n#EXT-X-PLAYLIST-TYPE:VOD\n'
    for i in range(num_segments):
        seg_len = min(HLS_SEG_DURATION, duration - i * HLS_SEG_DURATION)
        m3u8 += f'#EXTINF:{seg_len:.3f},\nseg_{i:05d}.ts\n'
    m3u8 += '#EXT-X-ENDLIST\n'

    # The transcode session starts lazily on the first segment request, so a
    # resume at t=600s spawns ffmpeg directly at that position instead of
    # transcoding from 0. Existing segments are reused as cache.
    from flask import Response
    return Response(m3u8, mimetype='application/vnd.apple.mpegurl', headers={
...
def r_hls_segment(vid, q, segment):
    ...
    if _hls_seg_ready(hls_dir, seg_num):
    ...
    # Wait for the running session if it will reach this segment soon;
    # otherwise (re)start the session right at the requested segment.
    with _hls_lock:
        pi = _hls_procs.get(key)
        restart = True
        if pi and pi['proc'].poll() is None:
            frontier = _hls_frontier(hls_dir, pi['start_seg'])
            head = frontier if frontier >= pi['start_seg'] else pi['start_seg'] - 1
            if pi['start_seg'] <= seg_num <= head + HLS_WAIT_AHEAD:
                restart = False
        if restart:
            try:
                _start_hls_session(key, src, q, hls_dir, seg_num)
            except Exception as e:
                logger.error(f"HLS session start failed [{key}]: {e}")
                abort(500)
        waiting_on = _hls_procs[key]['proc']
    ...
    while time.time() < deadline:
        ...
        with _hls_lock:
            pi = _hls_procs.get(key)
            alive = pi is not None and pi['proc'].poll() is None
        if alive and pi['proc'] is not waiting_on:
            # A newer seek replaced the session while we waited. Keep waiting
            # only if the new session will still produce our segment;
            # otherwise the client has abandoned this request — bail out
            # instead of holding the worker for 60s.
            frontier = _hls_frontier(hls_dir, pi['start_seg'])
            head = frontier if frontier >= pi['start_seg'] else pi['start_seg'] - 1
            if pi['start_seg'] <= seg_num <= head + HLS_WAIT_AHEAD:
                waiting_on = pi['proc']
                ...
                abort(409)
        if not alive:
            # Session ended — segment may have been flushed on exit.
            if _hls_seg_ready(hls_dir, seg_num):
                return _serve_segment(str(hls_dir), segment)
            logger.error(f"HLS session died before producing {segment} [{key}]")
            abort(404)
        time.sleep(0.05)

    logger.error(f"HLS timeout waiting for {segment} [{key}]")
    abort(504)
...
def r_hls_stop(vid, q):
    key = f"{vid}_{q}"
    with _hls_lock:
        info = _hls_procs.pop(key, None)
    if info:
        _kill_hls_proc(info['proc'])
    # Segments stay on disk as cache; _cleanup_hls GCs idle renditions.
    return jsonify({'ok': True})


# ─── Seek-preview storyboards ─────────────────────────────────────

_storyboard_jobs = set()
...
def _generate_storyboard(vid, src, duration):
    import subprocess, json as _json, math
    try:
        interval = max(5, math.ceil(duration / 100))
        count = max(1, math.ceil(duration / interval))
        rows = math.ceil(count / STORYBOARD_COLS)
        STORYBOARD_DIR.mkdir(parents=True, exist_ok=True)
        jpg = STORYBOARD_DIR / f'{vid}.jpg'
        tmp = STORYBOARD_DIR / f'{vid}.tmp.jpg'
        cmd = [
            'ffmpeg', '-hide_banner', '-loglevel', 'error',
            '-skip_frame', 'nokey', '-i', str(src),
            '-vf', (f'fps=1/{interval},scale={STORYBOARD_TILE_W}:-2,'
                    f'tile={STORYBOARD_COLS}x{rows}'),
            '-frames:v', '1', '-q:v', '6', '-y', str(tmp)
        ]
        r = subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, timeout=600)
        if r.returncode != 0 or not tmp.exists() or tmp.stat().st_size == 0:
            logger.error(f"Storyboard failed for {vid}: {r.stderr.decode(errors='ignore')[-200:]}")
        ...
        w, h = (int(x) for x in probe.stdout.decode().strip().rstrip(',').split(','))
        tmp.rename(jpg)
        meta = {
            'interval': interval, 'count': count, 'cols': STORYBOARD_COLS,
            'tile_w': w // STORYBOARD_COLS, 'tile_h': h // rows,
        }
        (STORYBOARD_DIR / f'{vid}.json').write_text(_json.dumps(meta))
        logger.info(f"Storyboard ready for {vid}: {count} tiles @ {interval}s")
    except Exception as e:
        logger.error(f"Storyboard error for {vid}: {e}")
    finally:
        with _storyboard_lock:
            _storyboard_jobs.discard(vid)


@app.route('/api/videos/<vid>/storyboard')
def r_storyboard_meta(vid):
    import json as _json
    meta_file = STORYBOARD_DIR / f'{vid}.json'
    if meta_file.exists() and (STORYBOARD_DIR / f'{vid}.jpg').exists():
        meta = _json.loads(meta_file.read_text())
        meta['url'] = f'/api/videos/{vid}/storyboard.jpg'
        return jsonify(meta)
    ...
    with _storyboard_lock:
        if vid not in _storyboard_jobs:
            _storyboard_jobs.add(vid)
            threading.Thread(target=_generate_storyboard,
                             args=(vid, src, duration), daemon=True).start()
    return jsonify({'status': 'generating'}), 202


@app.route('/api/videos/<vid>/storyboard.jpg')
def r_storyboard_img(vid):
...
if __name__ == '__main__':
    ...
    scheduler.add_job(_smart_retention, 'interval', days=1, id='smart_retention', replace_existing=True)
    scheduler.add_job(_cleanup_hls, 'interval', hours=1, id='hls_gc', replace_existing=True)
    from waitress import serve as waitress_serve
    logger.info("Serving on 0.0.0.0:5000 (waitress)")
    waitress_serve(app, host='0.0.0.0', port=5000, threads=16, channel_timeout=120)
